...
class rackhd11_api_catalogs(fit_common.unittest.TestCase):
    def test_api_11_catalogs(self):
        ...
        logs.debug('examining %d entries from catalogs', len(api_data['json']))
        for item in api_data['json']:
            # check required fields
            req_fields = ['createdAt', 'node', 'source', 'updatedAt', 'data']
            logs.debug_4('Checking catalog-id %s for required fields %s', item['id'], req_fields)
            for subitem in req_fields:
                logs.debug_7("Checking catalog-id %s for existance of required field '%s'", item['id'], subitem)
                self.assertIn(subitem, item, subitem + ' field error')

    def test_api_11_nodes_ID_catalogs(self):
        # iterate through nodes
        logs.debug('Going to check catalogs in nodes %s', MON_NODES)
        for nodeid in MON_NODES:
            api_data = fit_common.rackhdapi("/api/1.1/nodes/" + nodeid + "/catalogs")
            self.assertEqual(api_data['status'], 200, 'Incorrect HTTP return code, expected 200, got:' + str(api_data['status']))
            logs.debug_6('examining %d catalogs from nodeid %s', len(api_data['json']), nodeid)
            for item in api_data['json']:
                self.assertNotEqual(item, '', 'Empty JSON Field')
                logs.debug_7("Checking source %s for nodeid %s", item['source'], nodeid)
