...
class rackhd11_api_skus(fit_common.unittest.TestCase):
    def setUp(self):
    ...
    def test_api_11_sku(self):
        api_data = fit_common.rackhdapi("/api/1.1/skus")
        self.assertEqual(api_data['status'], 200, 'Incorrect HTTP return code, expected 200, got:' + str(api_data['status']))
        for item in api_data['json']:
            self.assertEqual(fit_common.rackhdapi("/api/1.1/skus/" + item['id'])['status'],
                             200, 'Incorrect HTTP return code, expected 200, got:' + str(api_data['status']))

    def test_api_11_sku_post_get_delete(self):
        data_payload = {
                        "name": "test1",
                        "rules": [
                            {
        ...
        api_data = fit_common.rackhdapi("/api/1.1/skus", action="post", payload=data_payload)
        self.assertEqual(api_data['status'], 200, 'Incorrect HTTP return code')
        api_data = fit_common.rackhdapi("/api/1.1/skus/" + api_data['json']['id'])
        self.assertEqual(api_data['status'], 200, 'Incorrect HTTP return code, expected 200, got:' + str(api_data['status']))
        api_data = fit_common.rackhdapi("/api/1.1/skus/" + api_data['json']['id'],
                                           action="delete")
        self.assertEqual(api_data['status'], 204, 'Incorrect HTTP return code, expected 204, got:' + str(api_data['status']))

    def test_api_11_sku_post_patch_delete(self):
        data_payload = {
                        "name": "test2",
                        "rules": [
                            {
        ...
        api_data = fit_common.rackhdapi("/api/1.1/skus/" + api_data['json']['id'], action="patch", payload=patch_payload)
        self.assertEqual(api_data['status'], 200, 'Incorrect HTTP return code, expected 200, got:' + str(api_data['status']))
