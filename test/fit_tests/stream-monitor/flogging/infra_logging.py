...
class _GeventInfoFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        print >>sys.stderr, record, record.filename, record.pathname
...
class _LevelLoggerClass(Logger):
    ...
    def __getattr__(self, key):
        ...
        debug_0 ends up mapping to the int value 15 (DEBUG - 0 + 5)
        debug_5 ends up mapping to the int value 10 (DEBUG - 5 + 5)
        debug_9 ends up mapping to the int value 6  (DEBUG - 9 + 5)
        ...
        for adj in xrange(0,10):
            for lvl_key, lvl_value in lvl_copy.items():
                if isinstance(lvl_key, str) and lvl_key != 'NOTSET':
                    ...
                    new_val = lvl_value - adj + 5
                    # Note: we can't insert our overlap (DEBUG_5 == DEBUG)
                    # here without changing what _appears_ in the log to be the _5
