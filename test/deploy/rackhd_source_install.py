...
class rackhd_source_install(fit_common.unittest.TestCase):
    ...
    def test02_clone_rackhd_source(self):
        ...
        '''
        # this section is for future use when repo selection is enabled
        # clone modules
    ...
    def test03_run_ansible_installer(self):
        # remove previous installation
        fit_common.remote_shell('rm -rf ~/src')
        # run Ansible source installer
        self.assertEqual(fit_common.remote_shell(PROXYVARS +
    ...
    def test06_startup(self):
        ...
        self.assertEqual(fit_common.remote_shell("cd ~/;pm2 restart rackhd-pm2-config.yml > /dev/null 2>&1")
