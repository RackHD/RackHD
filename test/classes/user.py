...
Description: Defines a RackHD User account base class and methods. All account role types (Administrator, Operator, and
...
class User:
        ...
        if fit_common.AUTH_TOKEN == 'Unavailable' or fit_common.REDFISH_TOKEN == 'None':
            if not fit_common.get_auth_token():
                if fit_common.VERBOSITY >= 4:
                    print 'error, unable to obtain fit_common RackHD and Redfish API administrative tokens'
                return None
    ...
    def createRackHDUser(self):
        ...
        url = "https://{0}:{1}/api/2.0/users?auth_token={2}".format(fit_common.fitargs()['rackhd_host'],
        ...
        if r.status_code != 201:
            if fit_common.VERBOSITY >= 4:
                print 'error status code {0}, could not create RackHD user account'.format(r.status_code)
            return False
        return True
    ...
    def createRedfishUser(self):
        ...
        url = "https://{0}:{1}/redfish/v1/AccountService/Accounts".format(fit_common.fitargs()['rackhd_host'],
        ...
        if r.status_code != 201:
            if fit_common.VERBOSITY >= 4:
                print "error status code {0}, could not create Redfish user account '{1}'".format(r.status_code, self.username)
            return False
        return True
    ...
    def setRackHDToken(self, username, passwd):
        ...
        url = "https://{0}:{1}/login".format(fit_common.fitargs()['rackhd_host'], str(fit_common.fitports()['https']))
        try:
            r = requests.post(url, headers=headers, data=json.dumps(payload), timeout=User.timeout, verify=False)
        except requests.exceptions.RequestException as e:
            if fit_common.VERBOSITY >= 4:
        ...
        if r.status_code != 200:
            if fit_common.VERBOSITY >= 4:
                print "error status code {0}, unable to set user rackhd token".format(r.status_code)
            return None
    ...
    def setRedfishToken(self, username, passwd):
        ...
        url = "https://{0}:{1}/redfish/v1/SessionService/Sessions".format(fit_common.fitargs()['rackhd_host'],
                                                                          str(fit_common.fitports()['https']))
        try:
            r = requests.post(url, headers=headers, data=json.dumps(payload), timeout=User.timeout, verify=False)
        except requests.exceptions.RequestException as e:
        ...
        if r.status_code != 200:
            if fit_common.VERBOSITY >= 4:
                print "error status code {0}, unable to set user redfish token".format(r.status_code)
            return None
    ...
    def deleteRackHDUserAccount(self):
        ...
        if r.status_code != 204:
            if fit_common.VERBOSITY >= 4:
                print "error status code {0}, unable to delete RackHD user '{1}'".format(r.status_code, self.username)
            return None
    ...
    def deleteRedfishUserAccount(self):
        ...
        url = "https://{0}:{1}/redfish/v1/AccountService/Accounts/{2}".format(fit_common.fitargs()['rackhd_host'],
        ...
        if r.status_code != 204:
            if fit_common.VERBOSITY >= 4:
                print "error status code {0}, unable to delete Redfish user '{1}'".format(r.status_code, self.username)
            return None
