...
class OSInstallTests(object):
    ...
    def __wait_for_completion(self, node, graph_name, graph_instance):
        ...
        while True:
            if (current_time - start_time).total_seconds() > DEFAULT_TIMEOUT_SEC :
               raise TypeError('Timed out after {0} seconds'.format(DEFAULT_TIMEOUT_SEC))
               break
            Api().workflows_get()
            workflows = loads(self.__client.last_response.data)
            for w in workflows:
                # LOG.info('print w : {0}'.format(w))
                if (w.get('node') == id and w.get('injectableName') == graph_name 
                   and w.get('instanceId') == graph_instance) :
                   status = w.get('status')
    ...
    def __post_workflow(self, graph_name, nodes, body):
        ...
        if index >= len(nodes):
            raise TypeError('index is outside the array range index: {0} vs nodes len {1}'.format(index, len(nodes)))
            return
        ...
        if log_context is None:
            raise TypeError('Could not find logContext in {0}'.format(self.__get_data()))
            return
