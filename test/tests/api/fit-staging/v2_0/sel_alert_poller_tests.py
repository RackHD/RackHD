...
class SELPollerAlertTests(fit_common.unittest.TestCase):
    ...
    def test_single_entry(self):
        ...
        self.__run_impitool_command(bmc_ip, command)
    ...
    def test_full_sel(self):
        ...
        self.__qproc.match_on_routekey('polleralert-sel-update',
                                       min=available_sel_entries, max=available_sel_entries,
                                       routing_key='polleralert.sel.updated.#.{}.{}'.format(poller_id, node_id))
        ...
        self.__run_impitool_command(bmc_ip, "sel add ~/selError.txt")

        # wait for the results
        results = self._amqp_sp.finish(timeout=360)
        results[0].assert_errors(self)

    def __run_impitool_command(self, ip, command):
        ...
        self.assertEqual(ipmi_rsp_data['exitcode'], 0,
                         msg='ipmitool command fail. exit code:{}'
                         .format(ipmi_rsp_data['exitcode']))
    ...
    def __get_SelInfo(self, bmc_ip, command):
        ...
        selInfoUnprocessed = self.__run_impitool_command(bmc_ip, command)
        ...
        if node_list:
            ...
            for node in node_list:
                if node.get('type') == 'compute' and node.get('sku'):
                    ...
                    # Look only for a quanta node
                    if "Quanta" not in sku.get('name'):
                        continue
