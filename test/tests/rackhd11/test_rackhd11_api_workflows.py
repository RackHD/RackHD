...
class rackhd11_api_workflows(fit_common.unittest.TestCase):
    ...
    def test_api_11_workflows_waitOn_anyOf(self):
        data_payload = {
        ...
        setup_return = fit_common.rackhdapi("/api/1.1/workflows", action="put", payload=data_payload)
        
        if not MON_NODES:
            run_return = fit_common.rackhdapi("/api/1.1/nodes/{0}/workflows/?name=Graph.REST.call", action="post")['JSON']
            for taskItr in run_return.tasks:
                if run_return.tasks.taskItr.friendlyName == "final REST call":
                    self.assertEqual(run_return.tasks.taskItr.state, "pending", "unreachable task REST-call-final state should be pending but get "+ run_return.tasks.taskItr);
